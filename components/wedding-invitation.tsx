...
export function WeddingInvitation({ id, config: c }: { id: string; config: InvitationConfig }) {
  ...
  const coverStyle = c.coverPath ? {
    backgroundImage: `linear-gradient(color-mix(in srgb, ${c.theme.background} 72%, transparent), color-mix(in srgb, ${c.theme.background} 78%, transparent)), url('${mediaUrl(c.coverPath)}')`,
    backgroundSize: 'cover',
    backgroundPosition: 'center',
  } : undefined

  return <main className="mx-auto min-h-screen max-w-md overflow-hidden bg-card shadow-2xl" style={mainStyle}>
    {!open && <section style={coverStyle} className="fixed inset-0 isolate z-50 flex flex-col items-center justify-between overflow-hidden bg-[var(--invite-bg)] px-7 py-12 text-center text-[var(--invite-text)]">
    ...
    <div className="bg-[var(--invite-bg)] text-[var(--invite-text)]">
      <section className="flex min-h-screen flex-col items-center justify-center px-7 py-16 text-center"><p className="text-xs uppercase tracking-[.35em]">Dengan izin Allah</p><h1 className="mt-8 font-serif text-5xl leading-tight">{c.couple.bride}<span className="block text-3xl italic text-[var(--invite-primary)]">&</span>{c.couple.groom}</h1><p className="mt-8 max-w-sm leading-relaxed opacity-75">{c.couple.tagline}</p><div className="mt-10 rounded-full border border-current/20 px-6 py-3 text-sm font-semibold">{left}</div></section>
      ...
      {gallery.length > 0 && <section className="bg-card px-7 py-16 text-card-foreground"><p className="text-center text-xs uppercase tracking-[.3em] text-primary">Kenangan</p><h2 className="mt-3 text-center font-serif text-4xl">Galeri Kami</h2><div className="mt-8 grid grid-cols-2 gap-3">{gallery.map((path, index) => <img key={path} src={mediaUrl(path)} alt={`Kenangan pasangan ${index + 1}`} className="aspect-[4/5] w-full rounded-2xl object-cover" />)}</div></section>}
